...
setwd("/home/david_caceres/Exosomas/Exosomas2/small/Expresion_yRNA//")
...
matrixA<-rbindlist(list4,fill=TRUE)
matrixA[is.na(matrixA)] = 0 # Elimino los NA
matrixA<-matrixA %>%   # Colapso los datos
    group_by(name) %>% 
    summarise(across(starts_with("x"), ~sum(., na.rm = TRUE)))
write.table(matrixA, file = "matrix_yRNA", append = FALSE, quote = TRUE, sep = "\t ")
```
