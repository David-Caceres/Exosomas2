...
comparisons <- list(c("A","B"),c("A","C"),c("A","D"),c("B","C"))
g<-ggplot(cross2, aes(x=batch_shortname, y=Reads, color=batch_shortname)) +
...
stat.test <- cross2%>% 
  wilcox_test(Short_reads ~ batch_shortname, ref.group = "D") %>%
  adjust_pvalue() %>%
  add_significance("p.adj")
stat.test
...
stat.test <- cross2%>% 
  wilcox_test(Ultra_short_reads ~ batch_shortname, ref.group = "D") %>%
  adjust_pvalue() %>%
  add_significance("p.adj")
stat.test
