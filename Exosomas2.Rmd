...
ggplot(cross2.mds, aes(x = V1, y = V2, color = reads_total)) +
    geom_point(size = 3) + coord_fixed() + ggtitle("MDS Reads total") + theme_bw() + scale_color_gradient(low="white", high="dark blue")
...
# Hay columnas sin datos, tenemos que retirarlas

col=colSums(counts.keep)==0

unname(which(col==TRUE))

counts.keep<-counts.keep[, colSums(counts.keep != 0) > 0]

cross3<-cross3[-c(29,31,38,45,48,50,51,65,70,89,94,99,100,108,110,142,150,176,192, 203),]

setcolorder(counts.keep,cross3$sample)

# Compruebo que sean iguales

all.equal(colnames(counts.keep), cross3$sample)
...
cross3.mds <- merge(x = cross3,y = mds1,by.x='sample',by.y='row.names')
...
ggplot(cross3.mds, aes(x = V1, y = V2, color = batch_shortname)) +
    geom_point(size = 3) + coord_fixed() + ggtitle("MDS Batch cdna")  + theme_bw() + scale_color_igv()

ggplot(cross3.mds, aes(x = V1, y = V2, color = Gender)) +
    geom_point(size = 3) + coord_fixed() + ggtitle("MDS Sex cdna") + theme_bw() + scale_color_igv()

ggplot(cross3.mds, aes(x = V1, y = V2, color = Diagnosis)) +
    geom_point(size = 3) + coord_fixed() + ggtitle("MDS Diagnosis cdna") + theme_bw() + scale_color_igv()

ggplot(cross3.mds, aes(x = V1, y = V2, color = Center)) +
    geom_point(size = 3) + coord_fixed() + ggtitle("MDS Center")  + theme_bw() + scale_color_igv()

ggplot(cross3.mds, aes(x = V1, y = V2, color = Age)) +
    geom_point(size = 3) + coord_fixed() + ggtitle("MDS Age cdna") + theme_bw() + scale_color_gradient(low="white", high="red")
...
counts.keep1 <- matriz_genomic_tRNA[keep,]


# Matriz de diseño
design  <-  model.matrix(~0+cross4$Diagnosis)

# Objeto de DESseq

dds <- DESeqDataSetFromMatrix(counts.keep1,
colData = cross4,
design = design)
...
cross4.mds <- merge(x = cross4,y = mds2,by.x='sample',by.y='row.names')

```


```{r}
ggplot(cross4.mds, aes(x = V1, y = V2, color = batch_shortname)) +
    geom_point(size = 3) + coord_fixed() + ggtitle("MDS Batch Genomic tRNA") + stat_ellipse() + theme_bw() + scale_color_igv()

ggplot(cross4.mds, aes(x = V1, y = V2, color = Gender)) +
    geom_point(size = 3) + coord_fixed() + ggtitle("MDS Sex Genomic tRNA") + stat_ellipse() + theme_bw() + scale_color_igv()

ggplot(cross4.mds, aes(x = V1, y = V2, color = Diagnosis)) +
    geom_point(size = 3) + coord_fixed() + ggtitle("MDS Diagnosis Genomic tRNA") + stat_ellipse() + theme_bw() + scale_color_igv()

ggplot(cross4.mds, aes(x = V1, y = V2, color = Center)) +
    geom_point(size = 3) + coord_fixed() + ggtitle("MDS Center Genomic tRNA") + theme_bw() + scale_color_igv()

ggplot(cross4.mds, aes(x = V1, y = V2, color = Age)) +
    geom_point(size = 3) + coord_fixed() + ggtitle("MDS Age Genomic tRNA") + theme_bw() + scale_color_gradient(low="white", high="red")
# 
# ggplot(cross4.mds, aes(x = V1, y = V2, color = Reads)) +
#     geom_point(size = 3) + coord_fixed() + ggtitle("MDS Reads") + theme_bw() + scale_color_gradient(low="white", high="red")
# 
# ggplot(cross4.mds, aes(x = V1, y = V2, color = Short_reads)) +
#     geom_point(size = 3) + coord_fixed() + ggtitle("MDS Short reads") + theme_bw() + scale_color_gradient(low="white", high="red")
# 
# ggplot(cross4.mds, aes(x = V1, y = V2, color = Ultra_short_reads)) +
#     geom_point(size = 3) + coord_fixed() + ggtitle("MDS Ultra short reads") + theme_bw() + scale_color_gradient(low="white", high="red")
# 
# ggplot(cross4.mds, aes(x = V1, y = V2, color = Bacteria)) +
#     geom_point(size = 3) + coord_fixed() + ggtitle("MDS Bacteria") + theme_bw() + scale_color_gradient(low="white", high="dark blue")
# 
# ggplot(cross4.mds, aes(x = V1, y = V2, color = Virus)) +
#     geom_point(size = 3) + coord_fixed() + ggtitle("MDS Virus") + theme_bw() + scale_color_gradient(low="white", high="dark blue")
```
...
setcolorder(matriz_hairpin,cross5$sample)

# Compruebo que sean iguales

all.equal(colnames(matriz_hairpin), cross5$sample)
...
counts.CPM <- cpm(matriz_hairpin)
thresh <- counts.CPM > 0.5

# Me quedo con los que pasan el umbral
keep <- rowSums(thresh) >= 10
counts.keep2 <- matriz_hairpin[keep,]

# Matriz de diseño
design  <-  model.matrix(~0+cross5$Diagnosis)

# Objeto de DESseq

dds <- DESeqDataSetFromMatrix(counts.keep2,
colData = cross5,
design = design)
...
mds4 <- cmdscale(dist(t(assay(vsd))))
cross5.mds <- merge(x = cross5,y = mds4,by.x='sample',by.y='row.names')

```

```{r}
ggplot(cross5.mds, aes(x = V1, y = V2, color = batch_shortname)) +
    geom_point(size = 3) + coord_fixed() + ggtitle("MDS Batch hairpin") + stat_ellipse() + theme_bw() + scale_color_igv()

ggplot(cross5.mds, aes(x = V1, y = V2, color = Gender)) +
    geom_point(size = 3) + coord_fixed() + ggtitle("MDS Sex hairpin") + stat_ellipse() + theme_bw() + scale_color_igv()

ggplot(cross5.mds, aes(x = V1, y = V2, color = Diagnosis)) +
    geom_point(size = 3) + coord_fixed() + ggtitle("MDS Diagnosis hairpin") + stat_ellipse() + theme_bw() + scale_color_igv()

ggplot(cross5.mds, aes(x = V1, y = V2, color = Center)) +
    geom_point(size = 3) + coord_fixed() + ggtitle("MDS Center hairpin") + theme_bw() + scale_color_igv()

ggplot(cross5.mds, aes(x = V1, y = V2, color = Age)) +
    geom_point(size = 3) + coord_fixed() + ggtitle("MDS Age hairpin") + theme_bw() + scale_color_gradient(low="white", high="red")

# ggplot(cross5.mds, aes(x = V1, y = V2, color = Reads)) +
#     geom_point(size = 3) + coord_fixed() + ggtitle("MDS Reads") + theme_bw() + scale_color_gradient(low="white", high="red")
# 
# ggplot(cross5.mds, aes(x = V1, y = V2, color = Short_reads)) +
#     geom_point(size = 3) + coord_fixed() + ggtitle("MDS Short reads") + theme_bw() + scale_color_gradient(low="white", high="red")
# 
# ggplot(cross5.mds, aes(x = V1, y = V2, color = Ultra_short_reads)) +
#     geom_point(size = 3) + coord_fixed() + ggtitle("MDS Ultra short reads") + theme_bw() + scale_color_gradient(low="white", high="red")
# 
# ggplot(cross5.mds, aes(x = V1, y = V2, color = Bacteria)) +
#     geom_point(size = 3) + coord_fixed() + ggtitle("MDS Bacteria") + theme_bw() + scale_color_gradient(low="white", high="dark blue")
# 
# ggplot(cross5.mds, aes(x = V1, y = V2, color = Virus)) +
#     geom_point(size = 3) + coord_fixed() + ggtitle("MDS Virus") + theme_bw() + scale_color_gradient(low="white", high="dark blue")
```
...
setcolorder(matriz_ncRNA,cross6$sample)

# Compruebo que sean iguales

all.equal(colnames(matriz_ncRNA), cross6$sample)
...
counts.CPM <- cpm(matriz_ncRNA)
thresh <- counts.CPM > 0.5

# Me quedo con los que pasan el umbral
keep <- rowSums(thresh) >= 10
counts.keep3 <- matriz_ncRNA[keep,]

# Matriz de diseño
design  <-  model.matrix(~0+cross6$Diagnosis)

# Objeto de DESseq

dds <- DESeqDataSetFromMatrix(counts.keep3,
colData = cross6,
design = design)
...
mds5 <- cmdscale(dist(t(assay(vsd))))

cross6.mds <- merge(x = cross6,y = mds5,by.x='sample',by.y='row.names')
...
```{r}
ggplot(cross6.mds, aes(x = V1, y = V2, color = batch_shortname)) +
    geom_point(size = 3) + coord_fixed() + ggtitle("MDS Batch ncRNA") + stat_ellipse() + theme_bw() + scale_color_igv()

ggplot(cross6.mds, aes(x = V1, y = V2, color = Gender)) +
    geom_point(size = 3) + coord_fixed() + ggtitle("MDS Sex ncRNA") + stat_ellipse() + theme_bw() + scale_color_igv()

ggplot(cross6.mds, aes(x = V1, y = V2, color = Diagnosis)) +
    geom_point(size = 3) + coord_fixed() + ggtitle("MDS Diagnosis ncRNA") + stat_ellipse() + theme_bw() + scale_color_igv()

ggplot(cross6.mds, aes(x = V1, y = V2, color = Center)) +
    geom_point(size = 3) + coord_fixed() + ggtitle("MDS Center ncRNA") + theme_bw() + scale_color_igv()

ggplot(cross6.mds, aes(x = V1, y = V2, color = Age)) +
    geom_point(size = 3) + coord_fixed() + ggtitle("MDS Age ncRNA") + theme_bw() + scale_color_gradient(low="white", high="red")
```
...
setcolorder(matriz_RNAcentral,cross7$sample)

# Compruebo que sean iguales

all.equal(colnames(matriz_RNAcentral), cross7$sample)
...
counts.CPM <- cpm(matriz_RNAcentral)
thresh <- counts.CPM > 0.5

# Me quedo con los que pasan el umbral
keep <- rowSums(thresh) >= 2
counts.keep4 <- matriz_RNAcentral[keep,]

# Matriz de diseño
design  <-  model.matrix(~0+cross7$Diagnosis)

# Objeto de DESseq

dds <- DESeqDataSetFromMatrix(counts.keep4,
colData = cross7,
design = design)
...
mds6 <- cmdscale(dist(t(assay(vsd))))

cross7.mds <- merge(x = cross7,y = mds6,by.x='sample',by.y='row.names')

```


```{r}
ggplot(cross7.mds, aes(x = V1, y = V2, color = batch_shortname)) +
    geom_point(size = 3) + coord_fixed() + ggtitle("MDS Batch RNAcentral")  + theme_bw() + scale_color_igv()

ggplot(cross7.mds, aes(x = V1, y = V2, color = Gender)) +
    geom_point(size = 3) + coord_fixed() + ggtitle("MDS Sex RNAcentral")  + theme_bw() + scale_color_igv()

ggplot(cross7.mds, aes(x = V1, y = V2, color = Diagnosis)) +
    geom_point(size = 3) + coord_fixed() + ggtitle("MDS Diagnosis RNAcentral") + theme_bw() + scale_color_igv()

ggplot(cross7.mds, aes(x = V1, y = V2, color = Center)) +
    geom_point(size = 3) + coord_fixed() + ggtitle("MDS Center RNAcentral") + theme_bw() + scale_color_igv()

ggplot(cross7.mds, aes(x = V1, y = V2, color = Age)) +
    geom_point(size = 3) + coord_fixed() + ggtitle("MDS Age RNAcentral") + theme_bw() + scale_color_gradient(low="white", high="red")

# ggplot(cross7.mds, aes(x = V1, y = V2, color = Reads)) +
#     geom_point(size = 3) + coord_fixed() + ggtitle("MDS Reads") + theme_bw() + scale_color_gradient(low="white", high="red")
# 
# ggplot(cross7.mds, aes(x = V1, y = V2, color = Short_reads)) +
#     geom_point(size = 3) + coord_fixed() + ggtitle("MDS Short reads") + theme_bw() + scale_color_gradient(low="white", high="red")
# 
# ggplot(cross7.mds, aes(x = V1, y = V2, color = Ultra_short_reads)) +
#     geom_point(size = 3) + coord_fixed() + ggtitle("MDS Ultra short reads") + theme_bw() + scale_color_gradient(low="white", high="red")
# 
# ggplot(cross7.mds, aes(x = V1, y = V2, color = Bacteria)) +
#     geom_point(size = 3) + coord_fixed() + ggtitle("MDS Bacteria") + theme_bw() + scale_color_gradient(low="white", high="dark blue")
# 
# ggplot(cross7.mds, aes(x = V1, y = V2, color = Virus)) +
#     geom_point(size = 3) + coord_fixed() + ggtitle("MDS Virus") + theme_bw() + scale_color_gradient(low="white", high="dark blue")
```
...
setcolorder(matriz_vRNA,cross8$sample)

# Compruebo que sean iguales

all.equal(colnames(matriz_vRNA), cross8$sample)
...
counts.CPM <- cpm(matriz_vRNA)
thresh <- counts.CPM > 0.5

# Me quedo con los que pasan el umbral
keep <- rowSums(thresh) >= 2
counts.keep <- matriz_vRNA[keep,]

# Matriz de diseño
design  <-  model.matrix(~0+cross8$Diagnosis)
...
mds3 <- cmdscale(dist(t(assay(vsd))))

cross8.mds <- merge(x = cross8,y = mds3,by.x='sample',by.y='row.names')
...
stat.test_reads <- cross2%>% 
  wilcox_test(Reads ~ batch_shortname) %>%
  adjust_pvalue() %>%
...
ggboxplot(cross2, x = "batch_shortname", y = "Reads",
          ...
          legend="right")+stat_pvalue_manual(stat.test_reads, label = "p.adj",tip.length = 0.01)+
  labs(y= "Reads",x= "Batch",fill="batch_shortname")


# Puedo hacerlo respecto a un solo batch

stat.test <- cross2%>% 
  wilcox_test(Reads ~ batch_shortname,ref.group = "D") %>%
  adjust_pvalue() %>%
  add_significance("p.adj")
stat.test_reads
...
stat.test_age <- cross2%>% 
  wilcox_test(Age ~ batch_shortname) %>%
  adjust_pvalue() %>%
  add_significance("p.adj")
stat.test_age
...
ggboxplot(cross2, x = "batch_shortname", y = "Age",
          palette = "igv",fill="batch_shortname",order=c("A","B","C","D","E","F","G","H"),
          legend="right")+stat_pvalue_manual(stat.test_age, label = "p.adj",tip.length = 0.01)+
  labs(y= "Age",x= "Batch",fill="batch_shortname")
...
stat.test_short <- cross2%>% 
  wilcox_test(Short_reads ~ batch_shortname) %>%
  adjust_pvalue() %>%
  add_significance("p.adj")
stat.test_short
...
ggboxplot(cross2, x = "batch_shortname", y = "Short_reads",
          palette = "igv",fill="batch_shortname",order=c("A","B","C","D","E","F","G","H"),
          legend="right")+stat_pvalue_manual(stat.test_short, label = "p.adj",tip.length = 0.01)+
  labs(y= "Short reads",x= "Batch",fill="batch_shortname")
...
stat.test_ushort <- cross2%>% 
  wilcox_test(Ultra_short_reads ~ batch_shortname) %>%
  adjust_pvalue() %>%
  add_significance("p.adj")
stat.test_ushort
...
ggboxplot(cross2, x = "batch_shortname", y = "Ultra_short_reads",
          palette = "igv",fill="batch_shortname",order=c("A","B","C","D","E","F","G","H"),
          legend="right")+stat_pvalue_manual(stat.test_ushort, label = "p.adj",tip.length = 0.01)+
  labs(y= "Ultra short reads",x= "Batch",fill="batch_shortname")
...
ggboxplot(cross2, x = "batch_shortname", y = "reads_total",
          palette = "igv",fill="batch_shortname",order=c("A","B","C","D","E","F","G","H"),
          legend="right")+stat_pvalue_manual(stat.test_reads_total, label = "p.adj",tip.length = 0.01)+
  labs(y= "Reads total",x= "Batch",fill="batch_shortname")
...
columnas<-c("Kidney_Abnormal.Creatinine","Kidney_Abnormal.lipid.profile","Kidney_Abnormal.urine.analysis","Kidney_Biopsy.proven.nephritis","Kidney_Proteinuria","Kidney_Urine.proteins","Lab_Reduced.C3.levels","Lab_Reduced.C4.levels","Symptom_Abnormal.inflammatory.indexes","Symptom_Disease.activity","Sampling_Patient.ID")

cross_sec2<-Cross_sec[,(colnames(Cross_sec) %in% columnas)]

colnames(cross_sec2)[colnames(cross_sec2) == "Sampling_Patient.ID"] <- "PatientID"

cross2<-merge(cross2, cross_sec2, by="PatientID")

colnames(cross2)[colnames(cross2) == "Gender"] <- "Sex"
