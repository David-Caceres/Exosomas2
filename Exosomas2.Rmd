...
cross_sec5<- as.data.frame(matrix(0, ncol = 1, nrow = 2620))

cross_sec5$Sex<-as.numeric(revalue(cross_sec2$Sex, c("Female"=1, "Male"=0)))

cross_sec5$Symptom_Abnormal.inflammatory.indexes2<-as.numeric(revalue(cross_sec2$Symptom_Abnormal.inflammatory.indexes, c("No"=-2, "Unknown"=0,"Past"=-1,"Present"=1)))

cross_sec5$Kidney_Abnormal.Creatinine2<-as.numeric(revalue(cross_sec2$Kidney_Abnormal.Creatinine,c("No"=-2, "Unknown"=0, "Past"=-1,"Present"=1))) 

cross_sec5$Kidney_Abnormal.lipid.profile2<-as.numeric(revalue(cross_sec2$Kidney_Abnormal.lipid.profile, c("No"=-2, "Unknown"=0, "Past"=-1,"Present"=1)))

cross_sec5$Kidney_Abnormal.urine.analysis2<-as.numeric(revalue(cross_sec2$Kidney_Abnormal.urine.analysis,c("No"=-2, "Unknown"=0,"Past"=-1,"Present"=1)))

cross_sec5$Kidney_Proteinuria2<-as.numeric(revalue(cross_sec2$Kidney_Proteinuria,c("No"=-2, "Unknown"=0, "Past"=-1,"Moderate"=1,"Severe"=1)))

cross_sec5$Lab_Reduced.C3.levels2<-as.numeric(revalue(cross_sec2$Lab_Reduced.C3.levels, c("No"=-2, "Unknown"=0, "Past"=-1,"Present"=1)))

cross_sec5$Lab_Reduced.C4.levels2<-as.numeric(revalue(cross_sec2$Lab_Reduced.C4.levels, c("No"=-2, "Unknown"=0, "Past"=-1,"Present"=1)))

cross_sec5$Kidney_Urine.proteins<-as.numeric(revalue(cross_sec2$Kidney_Urine.proteins, c("No"=-2, "Unknown"=0, "Past"=-1,"Present"=1)))

cross_sec5$Proven_nephritis<-factor(cross_sec2$Kidney_Biopsy.proven.nephritis)

cross_sec5$PatientID<-factor(cross_sec2$PatientID)

cross_sec5$Diagnosis<-factor(cross_sec2$Diagnosis)

cross_sec5<-cross_sec5[-1]

# Tabla final

cross_sec6<-data_frame(cross_sec5$Proven_nephritis,cross_sec5$Sex,cross_sec5$Symptom_Abnormal.inflammatory.indexes2,cross_sec5$Kidney_Abnormal.Creatinine2, cross_sec5$Kidney_Abnormal.lipid.profile2,cross_sec5$Kidney_Abnormal.urine.analysis2,cross_sec5$Kidney_Urine.proteins,cross_sec5$Kidney_Proteinuria2,cross_sec5$Lab_Reduced.C3.levels2,cross_sec5$Lab_Reduced.C4.levels2, cross_sec5$Symptom_Disease.activity,cross_sec5$PatientID,cross_sec5$Diagnosis)

colnames(cross_sec6)<-c("Proven_nephritis","Sex", "Inflammatory", "Creatinine", "lipid", "Urine",  "Urine_Kdiney_Proteins","Proteinuria", "C3_levels", "C4_levels","PatientID","Diagnosis")


cross_sec7<-merge(cross2, cross_sec6, by="PatientID")

cross_sec7<-cross_sec7[-29]

names(cross_sec7)[names(cross_sec7) == 'Diagnosis.x'] <- 'Diagnosis'


cross_sec8<-cross_sec7[cross_sec7$Diagnosis == "SLE",]

cross_lupus<-cross_sec8[,19:28]


cross_sec9<- cross_sec6 %>% anti_join(cross_sec7, by="PatientID")

cross_no_micro_SLE<-as.data.frame(cross_sec9[cross_sec9$Diagnosis == "SLE",])

cross_no_micro_SLE<-cross_no_micro_SLE[,1:10]

cross_no_micro_SLE2<-cross_no_micro_SLE

cross_lupus2<-cross_lupus

cross_lupus<-cross_lupus[cross_lupus$Proven_nephritis != 'Unknown',]
cross_lupus$Proven_nephritis<-droplevels(cross_lupus$Proven_nephritis)

cross_lupus$Proven_nephritis<-droplevels(cross_lupus$Proven_nephritis)



cross_no_micro_SLE<-cross_no_micro_SLE[cross_no_micro_SLE$Proven_nephritis != 'Unknown',]
cross_no_micro_SLE$Proven_nephritis<-droplevels(cross_no_micro_SLE$Proven_nephritis)


cross_no_SLE<-cross_no_micro[cross_sec6$Diagnosis != 'SLE',]
cross_no_SLE<-data_frame(cross_no_SLE$Kidney_Biopsy.proven.nephritis, cross_no_SLE$Sex, cross_no_SLE$Symptom_Abnormal.inflammatory.indexes,cross_no_SLE$Kidney_Abnormal.Creatinine,cross_no_SLE$Kidney_Abnormal.lipid.profile,cross_no_SLE$Kidney_Abnormal.urine.analysis,cross_no_SLE$Kidney_Urine.proteins,cross_no_SLE$Kidney_Proteinuria,cross_no_SLE$Lab_Reduced.C3.levels,cross_no_SLE$Lab_Reduced.C4.levels)

cross_no_SLE$`cross_no_SLE$Sex`<-as.numeric(revalue(cross_no_SLE$`cross_no_SLE$Sex`, c("Female"=1, "Male"=0)))

colnames(cross_no_SLE)<-c("Proven_nephritis","Sex", "Inflammatory", "Creatinine", "lipid", "Urine",  "Urine_Kdiney_Proteins","Proteinuria", "C3_levels", "C4_levels")

write.table(cross_no_SLE, file = "/home/david_caceres/Exosomas/Exosomas2/Tablas/cross_no_SLE.tsv", append = FALSE, quote = TRUE, sep = "\t ")
cross_no_SLE<-read.table(file="/home/david_caceres/Exosomas/Exosomas2/Tablas/cross_no_SLE.tsv", header = TRUE, sep = "\t")

cross_no_SLE<-as.numeric(cross_no_SLE[,2:10])
```
...
library(doParallel)
TrainModelSet <- training[training$Proven_nephritis!='Unknown',]
TrainModelSet$Proven_nephritis <- factor(TrainModelSet$Proven_nephritis)


TrainModelSet <- upSample(x = TrainModelSet[, colnames(TrainModelSet)],
                          y = TrainModelSet$Proven_nephritis)                         
table(TrainModelSet$Proven_nephritis) 

TrainModelSet<-TrainModelSet[,-11]

trainIndex <- createDataPartition(TrainModelSet$Proven_nephritis, p = .75, 
                                  list = FALSE, 
                                  times = 20)
...
for (i in 1:ncol(trainIndex)) {
  models.sub[[i]] <- list()
  models.sub[[i]][["train"]] <- TrainModelSet[trainIndex[,i],]
  models.sub[[i]][["test"]] <- TrainModelSet[-trainIndex[,i],]
  cl <- makePSOCKcluster(10)
  registerDoParallel(cl)
  models.sub[[i]]$modelRF <- train(Proven_nephritis ~ .,
                                   data = models.sub[[i]]$train,
                                   method = 'rf',
                                   tuneLength = 10,
                                   trControl = trControl,
                                   metric = "ROC")
  stopCluster(cl)
  models.sub[[i]]$predRF <- predict(object = models.sub[[i]]$modelRF,
                                    newdata = models.sub[[i]][["test"]],
                                    type = "prob")
  cl <- makePSOCKcluster(10)
  registerDoParallel(cl)
  models.sub[[i]]$modelKNN <- train(Proven_nephritis ~ .,
                                   data = models.sub[[i]]$train,
                                   method = 'knn',
                                   tuneLength = 10,
                                   trControl = trControl,
                                   metric = "ROC")
  stopCluster(cl)
  models.sub[[i]]$predKNN <- predict(object = models.sub[[i]]$modelKNN,
                                    newdata = models.sub[[i]][["test"]],
                                    type = "prob")
  cl <- makePSOCKcluster(10)
  registerDoParallel(cl)
  models.sub[[i]]$modelSVM <- train(Proven_nephritis ~ .,
                                   data = models.sub[[i]]$train,
                                   method = 'svmLinear',
                                   tuneLength = 10,
                                   trControl = trControl,
                                   metric = "ROC")
  stopCluster(cl)
  models.sub[[i]]$predSVM <- predict(object = models.sub[[i]]$modelSVM,
                                    newdata = models.sub[[i]][["test"]],
                                    type = "prob")
  cl <- makePSOCKcluster(10)
  registerDoParallel(cl)
  models.sub[[i]]$modelNB <- train(Proven_nephritis ~ .,
                                   data = models.sub[[i]]$train,
                                   method = 'naive_bayes',
                                   trControl = trControl,
                                   metric = "ROC")
  stopCluster(cl)
  models.sub[[i]]$predNB <- predict(object = models.sub[[i]]$modelNB,
                                    newdata = models.sub[[i]][["test"]],
                                    type = "prob")
}

dim(TrainModelSet[trainIndex[,1],])
dim(TrainModelSet[-trainIndex[,1],])
...
for (i in 1:20){
resultRF[[i]]<-models.sub[[i]]$modelRF$results[which.max(models.sub[[i]]$modelRF$results$ROC), ]
resultadosRF[[i]]<-resultRF[[i]][,c(1:2)]
}
resultadosRF<-rbindlist(resultadosRF)

resultSVM<-list()
resultadoSVM<-list()
for (i in 1:20){
resultSVM[[i]]<-models.sub[[i]]$modelSVM$results[which.max(models.sub[[i]]$modelSVM$results$ROC), ]
resultadoSVM[[i]]<-resultSVM[[i]][,c(1:2)]
}
resultadosSVM<-rbindlist(resultadoSVM)

resultKNN<-list()
resultadosKNN<-list()
for (i in 1:20){
resultKNN[[i]]<-models.sub[[i]]$modelKNN$results[which.max(models.sub[[i]]$modelKNN$results$ROC), ]
resultadosKNN[[i]]<-resultKNN[[i]][,c(1:2)]
}
resultadosKNN<-rbindlist(resultadosKNN)
...
resultadosNB<-list()
for (i in 1:20){
resultNB[[i]]<-models.sub[[i]]$modelNB$results[which.max(models.sub[[i]]$modelNB$results$ROC), ]
resultadosNB[[i]]<-resultNB[[i]][,c(3:4)]
resultadosNB[[i]]<-resultNB[[i]][,c(3:4)]
}
resultadosNB<-rbindlist(resultadosNB)


rocs<-cbind(resultadosRF,resultadosSVM,resultadosKNN,resultadosNB)
...
for (i in 1:ncol(trainIndex)) {
  models.sub2[[i]] <- list()
  models.sub2[[i]][["train"]] <- TrainModelSet[trainIndex[,i],]
  models.sub2[[i]][["test"]] <- TrainModelSet[-trainIndex[,i],]
  cl <- makePSOCKcluster(10)
  registerDoParallel(cl)
  models.sub2[[i]]$modelRF <- train(Proven_nephritis ~ .,
                                   data = models.sub2[[i]]$train,
                                   method = 'rf',
                                   tuneLength = 10,
                                   trControl = trControl,
                                   metric = "ROC",
  ...
  stopCluster(cl)
  models.sub2[[i]]$predRF <- predict(object = models.sub2[[i]]$modelRF,
                                    newdata = models.sub2[[i]][["test"]],
                                    type = "prob")
  cl <- makePSOCKcluster(10)
  registerDoParallel(cl)
  models.sub2[[i]]$modelKNN <- train(Proven_nephritis ~ .,
                                   data = models.sub2[[i]]$train,
                                   method = 'knn',
                                   tuneLength = 10,
                                   trControl = trControl,
  ...
  stopCluster(cl)
  models.sub2[[i]]$predKNN <- predict(object = models.sub2[[i]]$modelKNN,
                                    newdata = models.sub2[[i]][["test"]],
                                    type = "prob")
  cl <- makePSOCKcluster(10)
  registerDoParallel(cl)
  models.sub2[[i]]$modelSVM <- train(Proven_nephritis ~ .,
                                   data = models.sub2[[i]]$train,
                                   method = 'svmLinear',
                                   tuneLength = 10,
                                   trControl = trControl,
                                   metric = "ROC")
  stopCluster(cl)
  models.sub2[[i]]$predSVM <- predict(object = models.sub2[[i]]$modelSVM,
                                    newdata = models.sub2[[i]][["test"]],
                                    type = "prob")
  cl <- makePSOCKcluster(10)
  registerDoParallel(cl)
  models.sub2[[i]]$modelNB <- train(Proven_nephritis ~ .,
                                   data = models.sub2[[i]]$train,
                                   method = 'naive_bayes',
                                   trControl = trControl,
                                   metric = "ROC")
  stopCluster(cl)
  models.sub2[[i]]$predNB <- predict(object = models.sub2[[i]]$modelNB,
                                    newdata = models.sub2[[i]][["test"]],
                                    type = "prob")
}
...
curva_roc_rf<-list()
for (i in 1:20){
curva_roc_rf[[i]] <- roc(response = models.sub2[[i]][["test"]]$Proven_nephritis, predictor = models.sub2[[i]]$predRF$Yes)
}
plot(curva_roc_rf[[1]], main= "Random Forest", col="red")
set.seed(5460)
color<-rainbow(n=20)
for (i in 1:20){

plot(curva_roc_rf[[i]], main= "Random Forest", add=TRUE, col=color[i])
  }

# Cálculo de la curva
curva_roc_svm<-list()
for (i in 1:20){
curva_roc_svm[[i]] <- roc(response = models.sub2[[i]][["test"]]$Proven_nephritis, predictor = models.sub2[[i]]$predSVM$Yes)
}

plot(curva_roc_svm[[1]], main= "SVM", col="red")
set.seed(5460)
color<-rainbow(n=20)
for (i in 1:20){

plot(curva_roc_svm[[i]], main= "SVM", add=TRUE, col=color[i])
  }
...
curva_roc_knn<-list()
for (i in 1:20){
curva_roc_knn[[i]] <- roc(response = models.sub2[[i]][["test"]]$Proven_nephritis, predictor = models.sub2[[i]]$predKNN$Yes)
}

plot(curva_roc_knn[[1]], main= "KNN", col="red")
set.seed(5460)
color<-rainbow(n=20)
for (i in 1:20){

plot(curva_roc_knn[[i]], main= "KNN", add=TRUE, col=color[i])
  }

# Cálculo de la curva
curva_roc_nb<-list()
for (i in 1:20){
curva_roc_nb[[i]] <- roc(response = models.sub2[[i]][["test"]]$Proven_nephritis, predictor = models.sub2[[i]]$predNB$Yes)
}

plot(curva_roc_nb[[1]], main= "NB", col="red")
set.seed(5460)
color<-rainbow(n=20)
for (i in 1:20){

plot(curva_roc_nb[[i]], main= "NB", add=TRUE, col=color[i])
  }
...
# Tabla AUC-ROC predicciones

```{r}
auc_rf<-list()
for (i in 1:20){
auc_rf[[i]]<-curva_roc_rf[[i]]$auc
}

auc_rf<- do.call(rbind, auc_rf)

mtry_rf<-list()
for (i in 1:20){
mtry_rf[[i]]<-models.sub2[[i]]$modelRF$bestTune
}
mtry_rf<- do.call(rbind, mtry_rf)

tabla<-cbind(mtry_rf, auc_rf)

# SVM

auc_svm<-list()
for (i in 1:20){
auc_svm[[i]]<-curva_roc_svm[[i]]$auc
}

auc_svm<- do.call(rbind, auc_svm)

mtry_svm<-list()
for (i in 1:20){
mtry_svm[[i]]<-models.sub2[[i]]$modelSVM$bestTune
}
mtry_svm<- do.call(rbind, mtry_svm)

tabla<-cbind(mtry_rf,auc_rf,mtry_svm,auc_svm)
...
auc_knn<-list()
for (i in 1:20){
auc_knn[[i]]<-curva_roc_knn[[i]]$auc
}

auc_knn<- do.call(rbind, auc_knn)

mtry_knn<-list()
for (i in 1:20){
mtry_knn[[i]]<-models.sub2[[i]]$modelKNN$bestTune
}
mtry_knn<- do.call(rbind, mtry_knn)

tabla<-cbind(mtry_rf,auc_rf,mtry_svm,auc_svm,mtry_knn,auc_knn)
...
auc_nb<-list()
for (i in 1:20){
auc_nb[[i]]<-curva_roc_nb[[i]]$auc
}

auc_nb<- do.call(rbind, auc_nb)

mtry_nb<-list()
for (i in 1:20){
mtry_nb[[i]]<-models.sub2[[i]]$modelNB$bestTune
}
mtry_nb<-mtry_nb[[i]]$adjust
mtry_nb<- do.call(rbind, mtry_nb)

tabla<-cbind(mtry_rf,auc_rf,mtry_svm,auc_svm,mtry_knn,auc_knn,mtry_nb,auc_nb)
...
pred_RF2 <- predict(object = models.sub2[[13]]$modelRF,
                  newdata = models.sub2[[13]][["test"]],
                  type = "raw")



confusionMatrix(data=pred_RF2,reference=models.sub2[[13]]$test$Proven_nephritis, positive = "Yes")


pred_RF3 <- predict(object = models.sub2[[13]]$modelRF,
                  newdata = models.sub2[[13]][["test"]],
                  type = "prob")

par(mfrow=c(1,1))

curva_roc_rf3 <- roc(response = models.sub2[[13]]$test$Proven_nephritis, predictor = pred_RF3$Yes)

plot(curva_roc_rf3, main= "Random Forest", col="red", print.auc=TRUE)



trellis.par.set(caretTheme())
modelImp2 <- varImp(models.sub2[[13]]$modelRF, scale = FALSE)
plot(modelImp2, top = 9)
...
# Modelo frente a lupus sin micros


```{r}


predRF_SLE_NM <- predict(object = models.sub2[[13]]$modelRF,
                  newdata = cross_no_micro_SLE2,
                  type = "raw")

cross_no_micro_SLE2$Pred<-predRF_SLE_NM


label<-c()
for (i in 1:nrow(cross_no_micro_SLE2)){
if(cross_no_micro_SLE2$Proven_nephritis[i] == "Yes" & cross_no_micro_SLE2$Pred[i]=="No"){
label<-c(label,"Remision")}
else if(cross_no_micro_SLE2$Proven_nephritis[i] == "No" & cross_no_micro_SLE2$Pred[i]=="Yes"){
label<-c(label,"Early")}
else if(cross_no_micro_SLE2$Proven_nephritis[i] == "Yes" & cross_no_micro_SLE2$Pred[i]=="Yes"){
label<-c(label,"Established")}
else if(cross_no_micro_SLE2$Proven_nephritis[i] == "No" & cross_no_micro_SLE2$Pred[i]=="No"){
label<-c(label,"Negative")}
else if(cross_no_micro_SLE2$Proven_nephritis[i] == "Unknown" & cross_no_micro_SLE2$Pred[i]=="Yes"){
label<-c(label,"Posible Early")}
else if(cross_no_micro_SLE2$Proven_nephritis[i] == "Unknown" & cross_no_micro_SLE2$Pred[i]=="No"){
label<-c(label,"Negative")}
}

cross_no_micro_SLE2$Label<-label

```

# Modelo frente a lupus con micros


```{r}


predRF_SLE_M <- predict(object = models.sub2[[13]]$modelRF,
                  newdata = cross_lupus2,
                  type = "raw")

cross_lupus2$Pred<-predRF_SLE_M

label<-c()
for (i in 1:nrow(cross_lupus2)){
if(cross_lupus2$Proven_nephritis[i] == "Yes" & cross_lupus2$Pred[i]=="No"){
label<-c(label,"Remision")}
else if(cross_lupus2$Proven_nephritis[i] == "No" & cross_lupus2$Pred[i]=="Yes"){
label<-c(label,"Early")}
else if(cross_lupus2$Proven_nephritis[i] == "Yes" & cross_lupus2$Pred[i]=="Yes"){
label<-c(label,"Established")}
else if(cross_lupus2$Proven_nephritis[i] == "No" & cross_lupus2$Pred[i]=="No"){
label<-c(label,"Negative")}
else if(cross_lupus2$Proven_nephritis[i] == "Unknown" & cross_lupus2$Pred[i]=="Yes"){
label<-c(label,"Posible Early")}
else if(cross_lupus2$Proven_nephritis[i] == "Unknown" & cross_lupus2$Pred[i]=="No"){
label<-c(label,"Negative")}
}


cross_lupus2$Label<-label
...
```{r}
test2<-test[test$Proven_nephritis %like% "Unknown",]
test3<-test[test$Proven_nephritis != 'Unknown',]

write.table(test3, file = "/home/david_caceres/Exosomas/Exosomas2/Tablas/test3.tsv", append = FALSE, quote = TRUE, sep = "\t ")
test3<-read.table(file="/home/david_caceres/Exosomas/Exosomas2/Tablas/test3.tsv", header = TRUE, sep = "\t")
write.table(test2, file = "/home/david_caceres/Exosomas/Exosomas2/Tablas/test2.tsv", append = FALSE, quote = TRUE, sep = "\t ")
test2<-read.table(file="/home/david_caceres/Exosomas/Exosomas2/Tablas/test2.tsv", header = TRUE, sep = "\t")


test3$Proven_nephritis<-as.factor(test3$Proven_nephritis)
test2$Proven_nephritis<-as.factor(test2$Proven_nephritis)

library(plyr)
test3$Proven_nephritis<-revalue(test3$Proven_nephritis, c(" No"="No", " Yes" = "Yes"))
test2$Proven_nephritis<-revalue(test2$Proven_nephritis, c(" Unknown" = "Unknown"))
...
curva_roc_rf<-list()
for (i in 1:20){
curva_roc_rf[[i]] <- roc(response = models.sub[[i]][["test"]]$Proven_nephritis, predictor = models.sub[[i]]$predRF$Yes)
}
...
curva_roc_svm<-list()
for (i in 1:20){
curva_roc_svm[[i]] <- roc(response = models.sub[[i]][["test"]]$Proven_nephritis, predictor = models.sub[[i]]$predSVM$Yes)
}
...
curva_roc_knn<-list()
for (i in 1:20){
curva_roc_knn[[i]] <- roc(response = models.sub[[i]][["test"]]$Proven_nephritis, predictor = models.sub[[i]]$predKNN$Yes)
}
...
curva_roc_nb<-list()
for (i in 1:20){
curva_roc_nb[[i]] <- roc(response = models.sub[[i]][["test"]]$Proven_nephritis, predictor = models.sub[[i]]$predNB$Yes)
}
...
confusionMatrix(as.factor(test3$Proven_nephritis), pred, positive = "Yes")
...
pred2 <- predict(object = model_rf,
                        newdata = test3,
                        type = "raw")

confusionMatrix(reference=test3$Proven_nephritis,data=pred2, positive = "Yes")



trellis.par.set(caretTheme())
densityplot(model2, pch = "|")
modelImp2 <- varImp(model2, scale = FALSE)
plot(modelImp2, top = 8)

cal_obj <- calibration(Proven_nephritis ~ .,
                       data = train,
                       cuts = 10)
...
mtry_rf<-list()
for (i in 1:20){
mtry_rf[[i]]<-models.sub3[[i]]$modelRF$bestTune
}
mtry_rf<- do.call(rbind, mtry_rf)
...
mtry_svm<-list()
for (i in 1:20){
mtry_svm[[i]]<-models.sub3[[i]]$modelSVM$bestTune
}
mtry_svm<- do.call(rbind, mtry_svm)
...
mtry_knn<-list()
for (i in 1:20){
mtry_knn[[i]]<-models.sub3[[i]]$modelKNN$bestTune
}
mtry_knn<- do.call(rbind, mtry_knn)
...
mtry_nb<-list()
for (i in 1:20){
mtry_nb[[i]]<-models.sub3[[i]]$modelNB$bestTune
}
mtry_nb<-mtry_nb[[i]]$adjust
mtry_nb<- do.call(rbind, mtry_nb)
...
# Guardo y cargo los modelos.

```{r}

saveRDS(models.sub[[19]]$modelRF, "model_rf.rds")
model_rf <- readRDS("model_rf.rds")

saveRDS(models.sub[[18]]$modelSVM, "model_svm.rds")
model_svm <- readRDS("model_svm.rds")

saveRDS(models.sub[[16]]$modelKNN, "model_knn.rds")
model_knn <- readRDS("model_knn.rds")

saveRDS(models.sub[[1]]$modelNB, "model_nb.rds")
model_nb <- readRDS("model_nb.rds")
```



# Predicciones de los 4 mejores modelos
...
curva_roc_rf_mm <- roc(response = test$Proven_nephritis, predictor = pred_RF$Yes)

auc_rf_mm<-curva_roc_rf_mm$auc

mtry_rf_mm<-model_rf$bestTune

plot(curva_roc_rf_mm, main= "Random Forest", col="red", print.auc=TRUE)



pred_SVM <- predict(object = model_svm,
                  newdata = test,
                  type = "prob")

curva_roc_svm_mm <- roc(response = test$Proven_nephritis, predictor = pred_SVM$Yes)


auc_svm_mm<-curva_roc_svm_mm$auc

mtry_svm_mm<-model_svm$bestTune

plot(curva_roc_svm_mm, main= "SVM", col="red", print.auc=TRUE)


pred_KNN <- predict(object = model_knn,
                  newdata = test,
                  type = "prob")

curva_roc_knn_mm <- roc(response = test$Proven_nephritis, predictor = pred_KNN$Yes)


auc_knn_mm<-curva_roc_knn_mm$auc

mtry_knn_mm<-model_knn$bestTune

plot(curva_roc_knn_mm, main= "knn", col="red", print.auc=TRUE)



pred_RF <- predict(object = model_nb,
                  newdata = test,
                  type = "prob")

curva_roc_nb_mm <- roc(response = test$Proven_nephritis, predictor = pred_RF$Yes)


auc_nb_mm<-curva_roc_svm_mm$auc

mtry_nb_mm<-model_svm$bestTune

plot(curva_roc_nb_mm, main= "nb", col="red", print.auc=TRUE)

```


```{r}
pred_RF2 <- predict(object = model_rf,
                  newdata = test,
                  type = "raw")


pred_RF3 <- predict(object = model_rf,
                  newdata = test3,
                  type = "prob")


confusionMatrix(data=pred_RF2,reference=test$Proven_nephritis, positive = "Yes")


par(mfrow=c(1,1))

curva_roc_rf_mm <- roc(response = test3$Proven_nephritis, predictor = pred_RF3$Yes)

plot(curva_roc_nb_mm, main= "Random Forest", col="red", print.auc=TRUE)
...
library(doParallel)
  cl <- makePSOCKcluster(10)
  registerDoParallel(cl)
  model2RF <- train(Proven_nephritis ~ .,
                                   data = models.sub[[1]]$train,
                                   method = 'rf',
                                   tuneLength = 10,
                                   trControl = trControl,
                                   metric = "ROC")
  ...
  pred2RF <- predict(object = model2RF,
                                    newdata = models.sub[[15]][["test"]],
                                    type = "prob")
par(mfrow=c(1,2))
...
curva_rocRF <- roc(response = models.sub[[15]][["test"]]$Proven_nephritis, predictor = pred2RF$Yes)

auc_2rf<-curva_roc_RF$auc

mtry_2rf<-model2RF$bestTune

plot(curva_rocRF, main= "RF vs no micros", col="red", print.auc=TRUE)
  
  
# Evaluación con el dataset con micros.
  
  pred2RF <- predict(object = model2RF,
                                    newdata = test,
                                    type = "prob")
  
  curva_roc2RF<- roc(response = test$Proven_nephritis, predictor = pred2RF$Yes)


auc_2RF<-curva_rocRF$auc

mtry_2RF<-model2RF$bestTune

plot(curva_roc2RF, main= "RF vs micros", col="red", print.auc=TRUE)




pred_RF2 <- predict(object = model2RF,
                  newdata = test3,
                  type = "raw")
...
# Tabla de evaluacion de modelos
...
colnames(rocs)<-c("mtry","ROC_RF","C","ROC_SVM","k","ROC_KNN","adjust","ROC_NB")
...
trainIndex <- createDataPartition(training2$Proven_nephritis, p = .8, 
                                  list = FALSE, 
                                  times = )
  ...
  curva_roc4 <- roc(response = test9$Proven_nephritis, predictor = pred2RF$Yes)
  
  plot(curva_roc4, main= "RF", col="red", print.auc=TRUE)
  ...
  confusionMatrix(data=pred2RF,reference=test9$Proven_nephritis, positive = "Yes")
```
...
up_train <- upSample(x = training2[, colnames(training2)],
                     y = training2$Proven_nephritis)                         
table(up_train$Proven_nephritis) 

up_train<-up_train[,-11]

set.seed(6148)


trainIndex <- createDataPartition(up_train$Proven_nephritis, p = .8, 
                                  list = FALSE, 
                                  times = )
  ...
  curva_roc4 <- roc(response = test$Proven_nephritis, predictor = pred2RF$Yes)
  
  plot(curva_roc4, main= "RF", col="red", print.auc=TRUE)
  
  
    pred22RF <- predict(object =model2RF,
                                    newdata = test,
                                    type = "raw")
  
  confusionMatrix(data=pred22RF,reference=test$Proven_nephritis, positive = "Yes")
```
