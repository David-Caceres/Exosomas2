...
colnames(meta4)= c("sample", "batch","OMICID", "smp" )
...
y <- DGEList(counts=matriz2,group=cross2$batch)

# Elimino los contajes bajos

keep<- filterByExpr(y,group=cross2$batch)

table(keep)

# Devuelvo el resultado al dataframe anterior
...
col.status <- c("blue","red","dark green","yellow","brown","orange", "pink")[group]

plotMDS(y,col=col.status)

plotMDS(y,col=col.status, pch = pch[group])
```
...
```{r}
library(Rfast)
matriz3<-data.matrix(matriz2)
matriz3<-rowcvs(matriz3, ln = FALSE, unbiased = FALSE)
typeof(matriz3)
as.numeric(matriz3)


matriz4<-matriz2[order(matriz2$cv, decreasing = TRUE),]
