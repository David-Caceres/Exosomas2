...
matriz_cdna<-matriz_cdna[rowSums(matriz_cdna[, -1] > 0) != 0, ]
lol=colSums(matriz_cdna)==0
lol2=rowSums(matriz_cdna)==0
which(lol==TRUE)
which(lol2==TRUE)
...
dds <- DESeqDataSetFromMatrix(round(counts.keep/2),
colData = cross3,
design = design)
...
ggplot(cross3.mds, aes(x = V1, y = V2, color = batch_shortname)) +
    geom_point(size = 3) + coord_fixed() + ggtitle("MDS Batch cdna") + stat_ellipse() + theme_bw() + scale_color_igv()

ggplot(cross3.mds, aes(x = V1, y = V2, color = Gender)) +
    geom_point(size = 3) + coord_fixed() + ggtitle("MDS Sex cdna") + stat_ellipse() + theme_bw() + scale_color_igv()

ggplot(cross3.mds, aes(x = V1, y = V2, color = Diagnosis)) +
    geom_point(size = 3) + coord_fixed() + ggtitle("MDS Diagnosis cdna") + stat_ellipse() + theme_bw() + scale_color_igv()
...
# Me quedo con los que pasan el umbral
keep <- rowSums(thresh) >= 2
counts.keep <- matriz_genomic_tRNA[keep,]
...
# Me quedo con los que pasan el umbral
keep <- rowSums(thresh) >= 2
counts.keep <- matriz_hairpin[keep,]
...
cross5.mds <- merge(x = cross5,y = mds3,by.x='sample',by.y='row.names')
...
cross6.mds <- merge(x = cross6,y = mds3,by.x='sample',by.y='row.names')
...
# Me quedo con los que pasan el umbral
keep <- rowSums(thresh) >= 2
counts.keep <- matriz_RNAcentral[keep,]
...
cross7.mds <- merge(x = cross7,y = mds3,by.x='sample',by.y='row.names')
