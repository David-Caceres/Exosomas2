...
group <- paste(cross2$Age,cross2$Gender,cross2$Diagnosis,cross2$batch_shortname,  sep="\t" )
...
mds_plot<-ggplot(mds, aes(x = `1`, y = `2`, color = batch_shortname)) +
...
```{r}
cross3<-data.frame(cross2$Age,cross2$Gender,cross2$Diagnosis,cross2$batch_shortname, cross2$Reads,cross2$Short_reads,cross2$ushort_reads, cross2$bacteria_reads, cross2$virus_reads)
...
all.equal(colnames(logcounts_norm), rownames(cross3))
...
prince.plot(prince(com1,cross3,top=10))
